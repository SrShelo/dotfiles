...
if [ ! -f $FILE_FULL ] && [ $BAT_LVL -gt 95 ] && [ $BAT_DSCHARG -eq 0 ]; then
    notify-send "Battery charged" "Battery is fully charged." -i "/usr/share/icons/custom/battery-charging.png" -r $NOTIF_ID
    touch $FILE_FULL
elif [ ! -f $EMPTY_FILE ] && [ $BAT_LVL -le $WARNING_LVL ] && [ $BAT_DSCHARG -eq 1 ]; then
    notify-send "Low Battery" "Battery is almost empty!" -u critical -i "/usr/share/icons/custom/battery-alert.png" -r $NOTIF_ID
    touch $FILE_EMPTY
fi
