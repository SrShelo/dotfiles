...
if [ $BAT_CHARG -eq 1 ]; then
    NOTIF_ID=$(notify-send "Charging" "${BAT_LVL}% of battery charged." -u low -i "/usr/share/icons/custom/battery-plus.png" -t 5000 -r 9991 -p)
elif [ $BAT_CHARG -eq 0 ]; then    
    NOTIF_ID=$(notify-send "Discharging" "${BAT_LVL}% of battery remaining." -u low -i "/usr/share/icons/custom/battery-minus.png" -t 5000 -r 9991 -p)
fi
